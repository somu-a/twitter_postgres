...
for file in $files; do
    ...
    python3 -u load_tweets.py --db=postgres:pass@localhost:13101/ "--inputs=$file"
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalizedi
    unzip -p "$file" | sed 's/\\u0000//g' | psql postgressql://postgres:pass@localhost:13102 -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01 delimiter e'\x02';"
done
